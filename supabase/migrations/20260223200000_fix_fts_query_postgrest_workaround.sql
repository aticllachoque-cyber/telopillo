-- =============================================================================
-- Fix: PostgREST silently ignores fts_query parameter (schema cache / param resolution)
--
-- Workaround: Encode expanded FTS query in search_query itself.
-- - API passes search_query = expandQuery(q) ?? q (TypeScript synonym expansion)
-- - SQL detects tsquery format (contains " | " or " & ") and uses to_tsquery
-- - Otherwise uses plainto_tsquery for plain user input
--
-- This bypasses PostgREST's parameter handling entirely. No new params needed.
-- =============================================================================

-- Drop current signatures (with fts_query)
DROP FUNCTION IF EXISTS public.search_products(
  TEXT, TEXT, DECIMAL, DECIMAL, TEXT, TEXT, TEXT, TEXT, INT, INT, TEXT, TEXT
);

DROP FUNCTION IF EXISTS public.search_products_semantic(
  TEXT, VECTOR, TEXT, NUMERIC, NUMERIC, TEXT, TEXT, TEXT, TEXT, INT, INT, TEXT, TEXT
);

DROP FUNCTION IF EXISTS public.search_demands_hybrid(
  TEXT, VECTOR, TEXT, TEXT, TEXT, INT, INT, TEXT
);


-- =============================================================================
-- search_products: derive tsquery from search_query (no fts_query param)
-- =============================================================================
...
BEGIN
  ...
  IF v_has_search THEN
    -- If search_query contains tsquery operators (from expandQuery), use to_tsquery
    IF v_trimmed ~ ' \| | & ' THEN
...
-- =============================================================================
-- search_products_semantic: derive tsquery from search_query (no fts_query param)
-- =============================================================================
...
-- =============================================================================
-- search_demands_hybrid: derive tsquery from search_query (no fts_query param)
-- =============================================================================

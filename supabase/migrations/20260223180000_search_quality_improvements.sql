...
--
-- 1. Add fts_query parameter to all search functions (enables OR-based synonym expansion)
-- 2. Reduce semantic boost from 2x to 1.5x when FTS returns zero matches
-- 3. Tighten semantic thresholds: products 0.70, demands 0.65
-- 4. Fix demand triggers: add subcategory to column lists
-- 5. Update demand embedding text to include subcategory
-- =============================================================================


-- =============================================================================
-- 1. Drop old function signatures (different param count = different overload)
-- =============================================================================

DROP FUNCTION IF EXISTS public.search_products(
  TEXT, TEXT, DECIMAL, DECIMAL, TEXT, TEXT, TEXT, TEXT, INT, INT, TEXT
);

DROP FUNCTION IF EXISTS public.search_products_semantic(
  TEXT, VECTOR, TEXT, NUMERIC, NUMERIC, TEXT, TEXT, TEXT, TEXT, INT, INT, TEXT
);
...
-- =============================================================================
-- 2. Recreate search_products with fts_query parameter
-- =============================================================================
...
BEGIN
  ...
  IF v_has_search THEN
    IF fts_query IS NOT NULL AND trim(fts_query) != '' THEN
      v_tsquery := to_tsquery('spanish', fts_query);
    ELSE
  ...
  WITH filtered AS (
    ...
    WHERE
      ...
      AND (NOT v_has_search OR (p.search_vector @@ v_tsquery))
...
-- =============================================================================
-- 3. Recreate search_products_semantic with fts_query, 1.5x boost, 0.70 threshold
-- =============================================================================
...
DECLARE
  ...
  v_keyword_count INT;
  v_semantic_threshold CONSTANT DOUBLE PRECISION := 0.70;
BEGIN
  v_has_search := (search_query IS NOT NULL AND trim(search_query) != '');
...
-- =============================================================================
-- 4. Recreate search_demands_hybrid with fts_query, 1.5x boost, 0.65 threshold
-- =============================================================================
...
-- =============================================================================
-- 5. Fix demand triggers: add subcategory to column lists
-- =============================================================================

DROP TRIGGER IF EXISTS trg_demand_posts_search_vector ON public.demand_posts;
CREATE TRIGGER trg_demand_posts_search_vector
  BEFORE INSERT OR UPDATE OF title, description, category, subcategory
  ON public.demand_posts
  FOR EACH ROW
  EXECUTE FUNCTION public.update_demand_posts_search_vector();

-- Update embedding trigger function to include subcategory in text
...
DROP TRIGGER IF EXISTS trg_demand_post_embedding ON public.demand_posts;
...
-- =============================================================================
-- 6. Backfill demand posts search_vector (picks up subcategory via trigger)
-- =============================================================================

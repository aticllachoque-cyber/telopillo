// Stage 8 dark-mode axe scan — sign-off condition: verify fix contrast in dark theme.
...
for (const [name, viewport] of Object.entries(VIEWPORTS)) {
  ...
  console.log(`\n=== dark ${name} HTTP ${resp?.status()} — ${violations.length} violations ===`)
...
await browser.close()
console.log(failures === 0 ? '\nAXE DARK: CLEAN' : `\nAXE DARK: ${failures} violation node(s)`)
